...
const places=[
 ...
 {id:'terminal2',name:'Terminal 2',detail:'Northern corridor',lat:15.0121,lon:102.1058,routes:[['4','white / blue','#4d78bd'],['10','white / red / yellow','#e06b58'],['15','white / purple','#a56ac4'],['19','image-only source route','#b168c9']]},
...
const mapCenter={lat:14.9799,lon:102.0977}, mapScale=125;
const mapPoint=(lat,lon)=>({x:(lon-mapCenter.lon)*mapScale,z:-(lat-mapCenter.lat)*mapScale});
...
function mapRoad([a,b],width=0.22){const p1=mapPoint(a[0],a[1]),p2=mapPoint(b[0],b[1]),dx=p2.x-p1.x,dz=p2.z-p1.z,len=Math.hypot(dx,dz);const m=new THREE.Mesh(new THREE.BoxGeometry(len,.06,width),new THREE.MeshLambertMaterial({color:0x48566b}));m.position.set((p1.x+p2.x)/2,.08,(p1.z+p2.z)/2);m.rotation.y=-Math.atan2(dz,dx);world.add(m)}
mapRoad([[14.9753,102.0979],[15.0121,102.1058]]);mapRoad([[14.9753,102.0979],[14.9862,102.0734]]);mapRoad([[14.9753,102.0979],[14.9484,102.0865]]);mapRoad([[14.965,102.065],[15.005,102.07]],.16);mapRoad([[14.96,102.115],[15.02,102.115]],.16);
...
function landmark(p,i){const g=new THREE.Group();g.userData={id:p.id};const x=[-7,4,1,-3][i],z=[0,-6,6,4][i];g.position.set(x,0,z);
  const pos=mapPoint(p.lat,p.lon);g.position.set(pos.x,0,pos.z);
...
function select(id){const p=places.find(x=>x.id===id);selectedId=id;Object.entries(landmarkMeshes).forEach(([key,g])=>{g.scale.setScalar(key===id?1.18:1);g.userData.ring.visible=key===id});document.querySelectorAll('.place').forEach(x=>x.classList.toggle('active',x.dataset.id===id));document.querySelector('#routes').innerHTML='<h2>'+p.name+'</h2><p>'+p.detail+' · historical demo</p>'+p.routes.map(r=>'<div class="route"><b>สาย '+r[0]+'</b><span class="swatch" style="background:'+r[2]+'"></span><span>'+r[1]+'</span></div>').join('')+'<p class="hint">Approximate historical association; direction and current service not verified.</p>'}
document.querySelector('#places').innerHTML=places.map(p=>'<button class="place" data-id="'+p.id+'"><b>'+p.name+'</b><br><small>'+p.detail+'</small></button>').join('');document.querySelectorAll('.place').forEach(x=>x.onclick=()=>select(x.dataset.id));select('yamo');
